...
class ReferenceContext:
    ...
    def from_mapping(cls,payload):
        ...
        # Validate payload shape before policy flags so callers receive the most specific schema error.
        constraints=string_tuple(payload.get("project_constraints",()),"project_constraints")
        diversity=string_tuple(payload.get("diversity_dimensions",()),"diversity_dimensions")
        reference_ids=string_tuple(payload.get("reference_ids",()),"reference_ids")
        principles=string_tuple(payload.get("principles",()),"principles")
        study_targets=string_tuple(payload.get("study_targets",()),"study_targets")
        limitations=string_tuple(payload.get("limitations",()),"limitations")
        ...
        if contract.get("project_constraints_applied_to_retrieval") is True:
            raise ValueError("project constraints must never be applied to reference retrieval")
        if reference_ids and not provenance: raise ValueError("reference context with references requires provenance")
        return cls(query.strip(),identity.strip(),constraints,diversity,reference_ids,principles,study_targets,limitations,provenance)
