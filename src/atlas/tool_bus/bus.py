...
class ToolBus:
    ...
    def execute(self, job: ToolJob) -> ToolResult:
        ...
        if review.decision != SafetyDecision.ALLOW:
            ...
            event_type = (
                ToolEventType.SAFETY_APPROVAL_REQUIRED
                if review.decision == SafetyDecision.REQUIRE_APPROVAL
                else ToolEventType.SAFETY_DENIED
            )
            self._record(
                event_type,
                job,
        ...
        self._record(
            ...
            job,
            message="Safety policy allowed the job.",
            metadata={"safety_decision": review.decision.value},
        )
        ...
        if not adapter.verify():
            self._record(
                ...
                job,
                message=f"Adapter verification failed: {job.tool_name}",
            )
            result = ToolResult(
                success=False,
        ...
        final_event = ToolEventType.JOB_SUCCEEDED if result.success else ToolEventType.JOB_FAILED
        self._record(
            final_event,
            job,
    ...
    def get_job_events(self, job_id: str) -> List[ToolEvent]:
        """Return the event history for one job."""
