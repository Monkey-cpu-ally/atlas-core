...
class AgentStore:
    ...
    def save_project(self, project: AtlasProject) -> None:
        with self._connect() as connection:
            ...
            for task in project.tasks.values():
                self._save_task(connection, project.project_id, task)

    @staticmethod
    def _save_task(connection: sqlite3.Connection, project_id: str, task: AgentTask) -> None:
        connection.execute(
            """
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                INSERT OR REPLACE INTO agent_memory(
                    memory_key, owner, value_json, author, is_private
                ) VALUES (?, ?, ?, ?, ?)
                """,
                (entry.key, owner, json.dumps(entry.value), entry.author, int(entry.is_private)),
            )

    def load_memory(self, key: str, owner: str | None = None) -> MemoryEntry | None:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM agent_memory WHERE memory_key = ? AND owner = ?",
                (key, owner or ""),
            ).fetchone()
            if row is None:
                return None
    ...
    def save_message(self, message: AgentMessage) -> int:
        with self._connect() as connection:
            cursor = connection.execute(
                """
                INSERT INTO council_messages(sender, recipient, kind, subject, body, project_id)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    message.sender,
    ...
    def save_decision(
        ...
        vote_payload = [
            {"agent": vote.agent, "score": vote.score, "verdict": vote.verdict.value}
            for vote in votes
        ]
        with self._connect() as connection:
            cursor = connection.execute(
                ...
                INSERT INTO council_decisions(
                    project_id, domain, verdict, weighted_score, weights_json, votes_json
                ) VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    project_id,
