...
def test_minimal_home_connects_conversation_to_existing_navigation():
    source = MINIMAL_HOME.read_text(encoding="utf-8")
    assert "navigationTranscriptFor" in source
    assert 'command?.type !== "conversation"' in source
    assert "command.project?.title" in source
    assert 'command.persona !== "atlas"' in source
    assert "if (navigationTranscript) onVoiceCommand" in source
