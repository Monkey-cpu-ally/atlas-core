...
- [x] **P2 Graph Traversal** — `services/memory_bank.py::neighborhood` extended with `min_weight` filter (drops low-confidence edges). `routes/memory.py` exposes `GET /api/membank/graph/expand?subject=&depth=&min_weight=&limit_per_layer=` returning `{subject, depth, min_weight, nodes[], edges[], node_count, edge_count}` — vis-network / sigma.js / d3-force ready. Coexists with the legacy `/graph/around` route.
- [x] **P3 Sentinel Anomaly Detection** — new `services/anomaly.py` runs Welford's online algorithm per (device, telemetry-key); 10-sample warmup gate; |z| ≥ 3.0 (default sigma) trips `device.state.anomaly = {drifting_keys, z_scores, sigma_threshold, since, last_seen}`. Auto-cleared when readings fall back inside envelope. Telemetry rows tagged `anomaly` flow into MemoryBank for council retrieval. New `GET /api/robot/devices/{id}/envelope` returns the learned mean/stddev per key. `POST /devices/{id}/envelope/reset` is owner-only. New `POST /devices/{id}/ask-council` auto-fires `/api/persona/council/chat` with an anomaly-framed (or healthy-state) question and returns the full council fan-out (3 sub-voices + synthesis).
- [x] **P3 frontend** — `AtlasSentinel.js` chip flips amber (`is-anomaly` class · amber pulse animation) when `device.state.anomaly` is set. Popover gains an anomaly block showing drifting keys + z-scores, plus a `MessagesSquare`-icon "ask the council" button. Council reply renders inline in a sub-block (`data-testid="sentinel-council-reply-<name>"`).
- [x] **P4 paho-mqtt Adapter** — new `services/mqtt_bridge.py` — lazy-connect singleton client, daemon network thread, best-effort publish to `<MQTT_TOPIC_PREFIX>/devices/<id>/cmd`. **Dormant when `MQTT_BROKER_HOST` env unset** — REST surface (`/api/robot/*`) and HTTP-poll inbox path are UNCHANGED. `submit_command` calls `mqtt_bridge.publish_command` best-effort after EXECUTE; failure never blocks. New `GET /api/robot/mqtt/status` returns `{enabled, broker_host, broker_port, topic_prefix, client_id, connected, last_error}` for the HUD. `paho-mqtt==2.1.0` added to `requirements.txt`. Clean shutdown wired into FastAPI `@app.on_event("shutdown")`.
- [x] **P5 arXiv Integration** — `services/source_fetchers.py` adds `_fetch_arxiv` using arXiv's free Atom API (`export.arxiv.org/api/query?id_list=`); `_extract_arxiv_id` regex handles both new IDs (`YYMM.NNNNN`) and old (`subject-cls/YYMMNNN`); `_parse_arxiv_entry` is a regex-based Atom parser (no feedparser dep). `_fetch_academic` dispatcher routes `arxiv.org` → `_fetch_arxiv`, non-arXiv academic URLs → generic page scrape. arXiv URL re-ingest now back-fills `source_author` (fix to `_reinforce` so legacy records gain authors after the parser landed).
- [x] **Seed-idempotency fix** — `seed_if_needed()` now checks devices by NAME instead of `count_documents > 0` (test runs accumulated 70+ ephemeral devices which masked the seed devices behind the default `/api/robot/devices?limit=50`). HUD components (`AtlasSentinel.js`, `RobotPanel.js`) bumped to `?limit=200`.
- [x] **VERIFIED** `iteration_19.json` — 41/41 backend (11 phase8_quad + 26 regression + 4 phase8_extras) + frontend HUD smoke GREEN: Sentinel ribbon shows all 3 chips with AETHER amber-anomaly state, popover renders z-score block, ask-council button triggers real gpt-5.2 council fan-out and reply renders.
