...
const PERSONAS = [
  { id: "ajani", name: "Ajani", role: "Strategy, risk, and execution" },
  { id: "minerva", name: "Minerva", role: "Research, knowledge, and guidance" },
  { id: "hermes", name: "Hermes", role: "Engineering, systems, and diagnostics" },
];
  ...
  return (
    <section className="observatory" aria-label="ATLAS Observatory">
      ...
      <div className="observatory__status-strip" aria-label="ATLAS readiness">
        <div data-state={systemReady ? "good" : "watch"}>
          <span>System</span>
          <strong>{systemReady ? "Ready" : "Review"}</strong>
        </div>
        <div data-state={health.watch ? "watch" : "good"}>
          <span>Projects at risk</span>
          <strong>{health.watch}</strong>
        </div>
        <div data-state={awarenessItems.length ? "watch" : "good"}>
          <span>Active alerts</span>
          <strong>{awarenessItems.length}</strong>
        </div>
        <div data-state={githubStatus === "connected" ? "good" : "muted"}>
          <span>Verified GitHub events</span>
          <strong>{githubItems.length}</strong>
        </div>
      </div>
      ...
      <div className="observatory__grid">
        ...
        <section className="observatory-card observatory-card--knowledge" aria-label="Knowledge Bank status">
          <div className="observatory-card__topline">
            ...
            <b>{knowledgeProject ? `${knowledgeProject.progress}%` : "Not linked"}</b>
          </div>
          ...
          <div className="observatory__project-list">
            {activeProjects.map((project) => (
              <button type="button" key={project.id} onClick={() => onOpenProject?.(project)}>
                <div><strong>{project.title}</strong><span>{project.updated}</span></div>
