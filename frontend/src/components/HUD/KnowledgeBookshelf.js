...
export default function KnowledgeBookshelf({ aiColor }) {
  ...
  return (
    <div className="knowledge-bookshelf" data-testid="knowledge-bookshelf">
      <div className="kb-header"><div><div className="kb-kicker">ATLAS KNOWLEDGE BANK</div><h3 style={{ color: aiColor }}><BookOpen size={15} /> Knowledge Bookshelf</h3><p>Browse verified learning resources across the canonical ATLAS subjects.</p></div><button className="bp-btn" onClick={() => setReloadKey((n) => n + 1)} title="Refresh bookshelf"><RefreshCw size={12} /></button></div>
      <label className="kb-search"><Search size={13} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search books, papers, topics, authors…" /></label>
      <div className="kb-subject-tabs" aria-label="Knowledge subjects"><button className={`kb-tab ${subject === 'all' ? 'active' : ''}`} onClick={() => setSubject('all')} style={subject === 'all' ? { borderColor: aiColor, color: aiColor } : undefined}>All</button>{subjects.map((s) => { const id = subjectId(s); return <button key={id} className={`kb-tab ${subject === id ? 'active' : ''}`} onClick={() => setSubject(id)} style={subject === id ? { borderColor: aiColor, color: aiColor } : undefined}>{subjectName(s)}</button>; })}</div>
      ...
      {!loading && !error && filtered.length > 0 && <div className="kb-shelf-grid">{filtered.map((r, i) => <button key={r.id || r.resource_id || `${r.title}-${i}`} className="kb-book" onClick={() => setSelected(r)} style={{ '--book-accent': aiColor }}><span className="kb-book-spine" /><span className="kb-book-type">{r.type || r.resource_type || r.source || 'RESOURCE'}</span><strong>{r.title || 'Untitled resource'}</strong><span className="kb-book-author">{r.author || r.provider || r.subject || 'ATLAS'}</span></button>)}</div>}
