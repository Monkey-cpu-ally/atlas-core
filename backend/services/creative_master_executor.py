...
async def execute_master(request:ExecutionRequest)->ExecutionResult:
    ...
    if reference_context and council.get("reference_boundaries_verified") is not True: raise ValueError("master gate requires semantic Critic Council proof that reference boundaries passed")
    ...
    master_id=str(uuid.uuid4()); return ExecutionResult(request.artifact_id,{"master_id":master_id,"artifact_id":request.artifact_id,"approved":True,"status":"master","critic_council":council,"reference_boundaries_verified":reference_context is not None and council.get("reference_boundaries_verified") is True,"quality_evidence":{g:evidence[g] for g in applicable},"passed_gates":applicable},"creative-master-gate")
