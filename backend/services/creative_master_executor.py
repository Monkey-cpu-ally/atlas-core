...
When Reference Intelligence participated in production, the gate also requires proof
that project identity, constraints, and anti-imitation boundaries survived review.
"""
...
async def execute_master(request: ExecutionRequest) -> ExecutionResult:
    ...
    if reference_context and council.get("reference_context_verified") is not True:
        raise ValueError("master gate requires Critic Council verification of reference boundaries")
    ...
    applicable = payload.get("applicable_gates") or list(REQUIRED_GATES); applicable = [str(gate) for gate in applicable]
    ...
    missing = [gate for gate in applicable if gate not in evidence]; failed = [gate for gate in applicable if gate in evidence and not _passed(evidence[gate])]
    ...
    return ExecutionResult(request.artifact_id, {"master_id": master_id, "artifact_id": request.artifact_id, "approved": True, "status": "master", "critic_council": council, "reference_boundaries_verified": reference_context is not None and council.get("reference_context_verified") is True, "quality_evidence": {gate: evidence[gate] for gate in applicable}, "passed_gates": applicable}, "creative-master-gate")
