...
v1 contract:
  ...
  * The CA's private key IS stored on disk under a writable ATLAS data
    directory (or MTLS_CA_DIR if set) so subsequent registrations are
...
def _resolve_ca_dir() -> Path:
    """Resolve a writable persistent CA directory without assuming `/app`.

    An explicit ``MTLS_CA_DIR`` is authoritative and must be writable. When it
    is not set we prefer a repository-local data directory, then fall back to a
    process-local temporary directory for CI/read-only deployments.
    """
    configured = (os.environ.get("MTLS_CA_DIR") or "").strip()
    if configured:
        candidates = [Path(configured).expanduser()]
        strict = True
    else:
        candidates = [
            Path(__file__).resolve().parents[1] / "_data" / "mtls",
            Path(tempfile.gettempdir()) / "atlas-mtls",
        ]
        strict = False
    ...
    prefix = "Configured MTLS_CA_DIR is not writable" if strict else "No writable mTLS CA directory available"
    raise RuntimeError(prefix + ": " + "; ".join(errors))


_DEFAULT_DIR = _resolve_ca_dir()
...
def is_enforced() -> bool:
    """When True, protected device endpoints require a verified client cert."""
...
def ensure_ca() -> tuple[x509.Certificate, ec.EllipticCurvePrivateKey]:
    """First-run: mint a long-lived self-signed CA; subsequent calls load it."""
...
def issue_device_cert(device_id: str, device_name: str) -> Dict[str, str]:
    """Mint a fresh keypair + client certificate for one registered device."""
