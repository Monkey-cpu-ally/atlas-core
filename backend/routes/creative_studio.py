...
def _reference_payload(reference):
    return {"id": reference.reference_id, "title": reference.title, "kind": reference.kind, "category": reference.category, "study": list(reference.study), "disciplines": list(reference.disciplines), "techniques": list(reference.techniques), "strengths": list(reference.strengths), "study_targets": list(reference.study_targets), "limitations": list(reference.limitations), "provenance": list(reference.provenance), "relationships": list(reference.relationships)}

@router.get("/references")
async def list_references(q: str = Query(default="", max_length=120)):
    library = CreativeReferenceLibrary.load_default(); references = library.search(q)
...
async def synthesize_references(
    ...
    project_identity: str = Query(default="", max_length=240),
    project_constraints: list[str] = Query(default=[]),
):
    """Synthesize diverse transferable principles while keeping project identity and constraints authoritative."""
    library = CreativeReferenceLibrary.load_default()
    try:
        synthesis = library.synthesize(q, limit=limit, minimum_references=minimum_references, project_identity=project_identity, project_constraints=project_constraints)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    return {
        "query": synthesis.query,
        "project_identity": synthesis.project_identity,
        "project_constraints": list(synthesis.project_constraints),
        "diversity_dimensions": list(synthesis.diversity_dimensions),
        "references": [{**_reference_payload(m.reference), "score": m.score, "matched_terms": list(m.matched_terms)} for m in synthesis.references],
        "principles": list(synthesis.principles), "study_targets": list(synthesis.study_targets), "limitations": list(synthesis.limitations), "provenance": list(synthesis.provenance),
        "synthesis_contract": {"multi_reference": True, "deterministic": True, "principle_only": True, "provenance_preserved": True, "anti_imitation_boundaries_preserved": True, "project_identity_overrides_reference_influence": True, "project_constraints_applied_to_retrieval": True, "diversity_aware_selection": True},
    }
...
async def list_jobs(project_id: str | None = Query(default=None, max_length=160)):
    jobs = job_store.list(project_id=project_id); return {"total": len(jobs), "items": [asdict(job) for job in jobs]}
