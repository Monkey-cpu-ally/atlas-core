...
class PriorArtRequest(BaseModel): items: List[Dict[str,Any]]=Field(default_factory=list); conclusion: str=Field(min_length=3,max_length=5000)
...
class ExperimentDesignRequest(BaseModel):
    hypothesis_id: str; independent_variables: List[str]=Field(min_length=1); dependent_variables: List[str]=Field(min_length=1); controls: List[str]=Field(min_length=1)
    procedure: List[str]=Field(min_length=1); pass_fail_criteria: List[str]=Field(min_length=1); safety_constraints: List[str]=Field(default_factory=list); replication_target: int=2
class ResultRequest(BaseModel): result_type: str=Field(min_length=2,max_length=100); summary: str=Field(min_length=3,max_length=8000); measurements: Dict[str,Any]=Field(default_factory=dict); resulting_status: str
...
async def create_investigation(req: InvestigationRequest):
    try: record=engine.create_investigation(**req.model_dump()); await engine.persist(record); return record
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
@router.get("/investigations")
async def list_investigations(status: Optional[str]=None,knowledge_layer: Optional[str]=None): items=engine.list_investigations(status=status,knowledge_layer=knowledge_layer); return {"count":len(items),"items":items}
...
async def add_analogy(investigation_id: str,req: AnalogyRequest):
    try: item=engine.add_analogy(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
...
async def generate_candidate(investigation_id: str,req: CandidateHypothesisRequest):
    try: item=engine.generate_candidate_hypothesis(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
...
async def accept_candidate(investigation_id: str,candidate_id: str):
    try: item=engine.accept_candidate_hypothesis(investigation_id,candidate_id); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
...
async def add_hypothesis(investigation_id: str,req: HypothesisRequest):
    try: item=engine.add_hypothesis(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
...
async def challenge_hypothesis(investigation_id: str,req: ChallengeRequest):
    try: item=engine.challenge_active_hypothesis(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
...
async def assess_prior_art(investigation_id: str,req: PriorArtAssessmentRequest):
    try: item=engine.assess_candidate_prior_art(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
@router.post("/investigations/{investigation_id}/prior-art")
async def add_prior_art(investigation_id: str,req: PriorArtRequest):
    try: record=engine.add_prior_art(investigation_id,**req.model_dump()); await engine.persist(record); return record
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
@router.post("/investigations/{investigation_id}/evidence")
async def add_evidence(investigation_id: str,req: EvidenceRequest):
    try: record=engine.add_evidence(investigation_id,evidence=req.evidence); await engine.persist(record); return record
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
@router.post("/investigations/{investigation_id}/evidence-evaluations")
async def evaluate_evidence(investigation_id: str,req: EvidenceEvaluationRequest):
    try: item=engine.evaluate_investigation_evidence(investigation_id,conflicts=req.conflicts); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
@router.post("/investigations/{investigation_id}/experiment-designs")
async def design_experiment(investigation_id: str,req: ExperimentDesignRequest):
    try: item=engine.design_investigation_experiment(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
...
async def set_experiment_plan(investigation_id: str,req: ExperimentPlanRequest):
    try: record=engine.set_experiment_plan(investigation_id,**req.model_dump()); await engine.persist(record); return record
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
@router.post("/investigations/{investigation_id}/results")
async def record_result(investigation_id: str,req: ResultRequest):
    try: item=engine.record_result(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
@router.post("/investigations/{investigation_id}/replications")
async def record_replication(investigation_id: str,req: ReplicationRequest):
    try: item=engine.record_replication(investigation_id,**req.model_dump()); await engine.persist(engine.get_investigation(investigation_id)); return item
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
...
async def promote(investigation_id: str):
    try:
        ...
        await approval.persist_draft(draft); return {"investigation_id":investigation_id,"approval_discovery_id":draft["discovery_id"],"approval_status":draft["status"],"draft":draft}
    except engine.DiscoveryEngineError as exc: raise HTTPException(422,str(exc)) from exc
