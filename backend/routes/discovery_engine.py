...
class PriorArtRequest(BaseModel):
    ...
    conclusion: str = Field(min_length=3, max_length=5000)
...
class ResultRequest(BaseModel):
    result_type: str = Field(min_length=2, max_length=100)
...
async def health():
    return {
        ...
        "investigations": len(engine.list_investigations()),
        "knowledge_layers": sorted(engine.LAYERS),
        "statuses": sorted(engine.STATUSES),
        "rule": "Discovery candidates are hypotheses until evidence and Council review support promotion.",
    }
...
async def create_investigation(req: InvestigationRequest):
    try:
        record = engine.create_investigation(**req.model_dump())
        await engine.persist(record)
        return record
...
async def list_investigations(status: Optional[str] = None, knowledge_layer: Optional[str] = None):
    items = engine.list_investigations(status=status, knowledge_layer=knowledge_layer)
...
async def add_hypothesis(investigation_id: str, req: HypothesisRequest):
    try:
        hypothesis = engine.add_hypothesis(investigation_id, **req.model_dump())
        await engine.persist(engine.get_investigation(investigation_id))
        return hypothesis
    except engine.DiscoveryEngineError as exc:
        raise HTTPException(422, str(exc)) from exc
...
async def add_prior_art(investigation_id: str, req: PriorArtRequest):
    try:
        record = engine.add_prior_art(investigation_id, **req.model_dump())
        await engine.persist(record)
        return record
    except engine.DiscoveryEngineError as exc:
        raise HTTPException(422, str(exc)) from exc
...
async def add_evidence(investigation_id: str, req: EvidenceRequest):
    try:
        record = engine.add_evidence(investigation_id, evidence=req.evidence)
        await engine.persist(record)
        return record
    except engine.DiscoveryEngineError as exc:
        raise HTTPException(422, str(exc)) from exc
...
async def set_experiment_plan(investigation_id: str, req: ExperimentPlanRequest):
    try:
        record = engine.set_experiment_plan(investigation_id, **req.model_dump())
        await engine.persist(record)
        return record
    except engine.DiscoveryEngineError as exc:
        raise HTTPException(422, str(exc)) from exc
...
async def record_result(investigation_id: str, req: ResultRequest):
    try:
        result = engine.record_result(investigation_id, **req.model_dump())
        await engine.persist(engine.get_investigation(investigation_id))
        return result
    except engine.DiscoveryEngineError as exc:
        raise HTTPException(422, str(exc)) from exc
...
async def promote(investigation_id: str):
    try:
        draft = engine.promote_to_approval(investigation_id)
        record = engine.get_investigation(investigation_id)
        if record:
            await engine.persist(record)
        ...
        return {
            ...
            "approval_discovery_id": draft["discovery_id"],
