...
class ExperimentPlanRequest(BaseModel):
    objective: str = Field(min_length=3, max_length=5000); method: List[str] = Field(min_length=1)
    measurements: List[str] = Field(min_length=1); pass_fail_criteria: List[str] = Field(min_length=1)
    safety_constraints: List[str] = Field(default_factory=list)
class ResultRequest(BaseModel):
    result_type: str = Field(min_length=2, max_length=100); summary: str = Field(min_length=3, max_length=8000)
    measurements: Dict[str, Any] = Field(default_factory=dict); resulting_status: str
...
async def frontier_map(subject: List[str] = Query(default=[])):
    return engine.map_frontier(subjects=subject)
