...
def _resolve_upload_dir() -> str:
    ...
    `UPLOAD_DIR` is authoritative when explicitly configured. Otherwise we
    prefer the historical `/app/uploads` location and fall back to the OS
    temporary directory for CI, local development, and read-only containers.
    """
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        # Check file size before writing anything to disk or MongoDB.
        file.file.seek(0, 2)
        ...
        # Keep disk and MongoDB consistent if categorization or persistence
        # fails after the file has already been written.
